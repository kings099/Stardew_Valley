...
bool FarmMap::onMouseEvent(cocos2d::Event* event)
{
    ...
    if (mouseEvent) {
        // 获取鼠标的屏幕位置
        const auto mousePos = mouseEvent->getLocationInView();
        // 获取默认摄像机的世界坐标偏移
        const auto defaultCamera = Camera::getDefaultCamera();
        const auto cameraPosition = defaultCamera->getPosition();

        // 可视范围
        const auto visibleSize = Director::getInstance()->getVisibleSize();

        const float cameraOffset_x = cameraPosition.x - visibleSize.width / 2;
        const float cameraOffset_y = cameraPosition.y - visibleSize.height / 2;

        // 将屏幕坐标修正为相对于地图的绝对坐标
        const Vec2 mapPosition(mousePos.x + cameraOffset_x - _mapPosition.x,
            mousePos.y + cameraOffset_y - _mapPosition.y);
        // 转换为瓦片坐标
        const Vec2 tiledPos = absoluteToTile(mapPosition);
        CCLOG("tilePos:%f,%f", tiledPos.x, tiledPos.y);
        auto layer = _tileMap->getLayer("Back"); // 确保替换为你实际的图层名称
        const int tileGID = layer->getTileGIDAt(tiledPos);
